...
let usage focus =
  ...
  if showSection "tokenize" then
    cp "\focsvtestbench tokenize \fg-i \fcfile.csv\f0 [\fg-raw\f0] [\fg-lines\f0] [\fg-sep \fcseparator\f0] [\fg-empty\f0] [\fg-n \fcfieldcount\f0|\fg-same\f0] [\fg-echo \fcfile\f0]"
  ...
  if showDetails "tokenize" then
    ...
    cp "  \fg-echo \fcfile\f0        Echo the CSV content back to \fcfile\f0."
    cp ""
  if true then
    cp "Common options:"
    cp "  \fg-h             \f0Show help"
